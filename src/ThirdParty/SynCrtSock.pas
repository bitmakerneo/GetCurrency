...
{$ifdef USEWININET}
  ...
  public
    ...
    /// wrapper method to retrieve some content via an HTTP GET
    ...
    class function Get(const aURI: RawByteString): RawByteString;
...
begin
  if P=nil then
    ...
    while (S^<>#0) and (S^<>Sep) do
      inc(S);
    SetString(result,P,S-P); // Unicode ready
    if S^<>#0 then
     P := S+1 else
     P := nil;
  end;
...
begin
  ...
  if P<>nil then
    repeat
      ...
      SetString(aName,Beg,P-Beg); // Unicode ready
...
const
  ...
  XPOWEREDVALUE = 'Synopse mORMot '+SYNOPSE_FRAMEWORK_VERSION+' http://synopse.info';
...
const
  cHttps: array[Boolean] of AnsiString = ('', 's');
begin
  Result := 'http' + cHttps[Https] + '://' + Server + ':' + Port + Address;
end;

function TURI.URI: AnsiString;
begin
  Result := ToString;
end;
...
constructor TCrtSocket.Open(const aServer, aPort: RawByteString; aLayer: TCrtSocketLayer;
  aTimeOut: cardinal);
begin
  Create(aTimeOut); // default read timeout is 10 sec
    ...
    if P^<>#13 then // at least NCSA 1.3 does send a #10 only -> ignore #13
      if P^=#10 then begin
        if Line='' then // get line
          SetString(Line,tmp,P-tmp) // Unicode ready
        else
        begin
...
begin
  ...
  if Assigned(fInternalHttpServerRespList) then
  begin
  ...
  try
    ...
    if (Context.OutContent<>'') and (Context.OutContentType=HTTP_RESP_STATICFILE) then
      try
        ...
        try
          SetString(Context.fOutContent,nil,FileToSend.Size); // Unicode ready
  ...
  if P<>nil then
    repeat
      ...
      if PDeb<>P then begin // add any not void line
        if length(Headers)<=n then
          SetLength(Headers,n+10);
        SetString(Headers[n],PDeb,P-PDeb); // Unicode ready
        inc(n);
      end;
...
begin
  ...
  // set headers content
  SetString(result,nil,L); // Unicode ready
  D := pointer(result);
  for H := low(KNOWNHEADERS_NAME) to high(KNOWNHEADERS_NAME) do
...
var
  ...
  begin
    try
      ...
      Http.SendHttpResponse(fServer.fReqQueue, Req^.RequestId,0,Resp^,nil,bytesSent);
    except
      on Exception do
...
begin
  ...
  Context := THttpServerRequest.Create(fServer, fServer);
  try
    Context.fServer := fServer;
    ...
    if Req^.Verb in [low(VERB_TEXT)..high(VERB_TEXT)] then
      ...
      SetString(Context.fMethod,Req^.pUnknownVerb,Req^.UnknownVerbLength); // Unicode ready
    with Req^.Headers.KnownHeaders[reqContentType] do
      SetString(Context.fInContentType,pRawValue,RawValueLength); // Unicode ready
    with Req^.Headers.KnownHeaders[reqAcceptEncoding] do
      SetString(InAcceptEncoding,pRawValue,RawValueLength); // Unicode ready
    InCompressAccept := ComputeContentEncoding(fServer.fCompress,pointer(InAcceptEncoding));
    Context.fInHeaders := RetrieveHeaders(Req^);
    // retrieve body
    Context.fInContent := '';
    Context.RemoteAddress := Req.Address.pRemoteAddress; // IH
      ...
      if InContentLength<>0 then begin
        ...
        repeat
          ...
          if Win32MajorVersion>5 then // speed optimization for Vista+
            ...
            flags := 0;
          Err := Http.ReceiveRequestEntityBody(fServer.fReqQueue,Req^.RequestId,flags,
        ...
        if RawValueLength<>0 then begin
          SetString(InContentEncoding,pRawValue,RawValueLength); // Unicode ready
          for i := 0 to high(fServer.fCompress) do
            if fServer.fCompress[i].Name=InContentEncoding then begin
              fServer.fCompress[i].Func(Context.fInContent,false); // uncompress
              break;
            end;
        end;
      end;
    ...
    try
      ...
      Resp^.SetStatus(fServer.Request(Context),OutStatus);
      ...
      if fServer.fCompressAcceptEncoding<>'' then
        Resp^.AddCustomHeader(pointer(fServer.fCompressAcceptEncoding),Heads);
      if Context.OutContentType=HTTP_RESP_STATICFILE then begin
        ...
        try
          ...
          with Req^.Headers.KnownHeaders[reqRange] do
            if (RawValueLength>6) and IdemPChar(pRawValue,'BYTES=') and
              ...
              SetString(Range,pRawValue+6,RawValueLength-6); // need #0 end // Unicode ready
              R := pointer(Range);
          ...
          Http.SendHttpResponse(fServer.fReqQueue,Req^.RequestId,flags,Resp^,nil,bytesSent);
        finally
        ...
        if fServer.fCompress<>nil then begin
          ...
          if RawValueLength=0 then begin
            ...
            OutContentEncoding := CompressDataAndGetHeaders(InCompressAccept,
              fServer.fCompress,Context.OutContentType,Context.fOutContent);
        ...
        Err := Http.SendHttpResponse(fServer.fReqQueue,Req^.RequestId,0,Resp^,nil,bytesSent);
        if Err<>NO_ERROR then
...
function RequestCallback(lpThreadParameter: Pointer): Integer; stdcall;
var
  aGen: TRespondGen;
begin
  aGen := TRespondGen(lpThreadParameter);
  try
    aGen.Execute;
  finally
    aGen.Destroy;
...
function QueueUserWorkItem(func: TThreadStartRoutine;
  Context: Pointer;
...
begin
  ...
  NotifyThreadStart(self);
    ...
    repeat
      ...
      if ReqID = 0 then
        SetString(ReqBuf, nil, 16384+sizeof(HTTP_REQUEST)); // space for Req^ + 16 KB of headers
      FillChar(ReqBuf[1], sizeof(HTTP_REQUEST),0);
      Err := Http.ReceiveHttpRequest(fReqQueue,ReqID,0,PHTTP_REQUEST(ReqBuf)^,length(ReqBuf),bytesRead);
      if Terminated then
        break;
      case Err of
        NO_ERROR:
          begin
            aGen := TRespondGen.Create;
            aGen.fServer := self;
            aGen.ReqBuf := ReqBuf;
            QueueUserWorkItem(RequestCallback, aGen, WT_EXECUTELONGFUNCTION);
            ReqID := 0;
          end;
        ERROR_MORE_DATA: begin
          // input buffer was too small to hold the request headers
          // -> increase buffer size and call the API again
          ReqID := PHTTP_REQUEST(ReqBuf).RequestId;
          SetString(ReqBuf, nil, bytesRead);
        end;
...
begin
  ...
  if IdemPChar(P,'ETAG:') then
    Known := respEtag else
  if IdemPChar(P,'CACHE-CONTROL:') then
    Known := reqCacheControl else
  if IdemPChar(P,'VARY:') then
    Known := respVary else begin
    UnknownName := P;
...
class function TWinHttpAPI.Get(const aURI: RawByteString): RawByteString;
var URI: TURI;
...
begin
  ...
  if From(aURI) then
  try
    with Create(Server,Port,Https) do
    try
      Request(Address,'GET',0,'','','',headers,result);
    finally
      Free;
    end;
  except
...
begin // see http://msdn.microsoft.com/en-us/library/windows/desktop/aa383884
  ...
  try
    while (tmpLen>0) and (ord(err[tmpLen-1]) in [0..32,ord('.')]) do
      dec(tmpLen);
    SetString(msg,err,tmpLen); // Unicode ready
  finally
...
function WinHttpSetOption(hInternet: HInternet; dwOption: Cardinal;
...
begin
  ...
  if NoAutoRedirect then
  begin //Disable automatic redirects
    Flags := WINHTTP_DISABLE_REDIRECTS;
    WinHttpSetOption(fRequest, WINHTTP_OPTION_DISABLE_FEATURE, @Flags, SizeOf(Flags));
  end;

  if NoCookies then
  begin //Disable automatic addition of cookie headers to requests
